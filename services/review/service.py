...
class ReviewService:
    ...
    def build_daily_report(self, report_date: str) -> ReviewReport:
        ...
        research_only_symbols = tuple(
            symbol for symbol in AUTO_PAPER_RESEARCH_SYMBOLS if symbol not in execution_symbols
        )
        execution_positions = tuple(position for position in positions if position.symbol in execution_symbols)
        symbol_pnl = {
            symbol: sum(
                (position.realized_pnl or 0 for position in execution_positions if position.symbol == symbol),
                0,
            )
        ...
        research_only_explanations = [
            f"{symbol}: 研究覆盖，不计入自动执行绩效；"
            f"{', '.join(terminal_reasons_by_symbol.get(symbol, ['无执行授权']))}"
            for symbol in research_only_symbols
        ]
        total_pnl = sum((position.realized_pnl or 0 for position in execution_positions), 0)
        ...
        report = ReviewReport(
            ...
            deviation_analysis=[
                f"当日已平仓 {len(execution_positions)} 笔，已实现净损益 {total_pnl}",
        ...
        report.deviation_analysis.insert(0, f"自动执行绩效范围: {', '.join(execution_symbols) or 'none'}")
        report.deviation_analysis.extend(research_only_explanations)
