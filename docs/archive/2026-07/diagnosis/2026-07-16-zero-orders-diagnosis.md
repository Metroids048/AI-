...
> 已过期（2026-07-16）：本文件基于旧进程和旧提交推断，不能作为当前运行决策依据。当前本地代码已确认：LLM 供应商故障是 advisory，不会单独拒绝订单；`net_edge_after_cost` 使用的是小数收益率而不是 R 倍数；自动调度、观察通道与实际拒单分布请以 `2026-07-16-automatic-paper-order-remediation.md` 为准。
