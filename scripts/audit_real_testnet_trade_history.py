...
def _fetch_windowed(
    client: Any, method: str, symbols: Iterable[str], start: datetime, *, include_symbol: bool
...
def _local_facts(
    database_url: str,
) -> tuple[list[dict[str, Any]], dict[tuple[str, str], dict[str, Any]], dict[str, str], dict[str, Any]]:
    ...
    try:
        with Session(engine) as session:
            cycles = list(
                session.scalars(
                    select(V2ExecutionCycle).where(
                        V2ExecutionCycle.execution_mode == V2ExecutionMode.BINANCE_TESTNET.value
                    )
                )
            )
            ...
            orders = (
                list(session.scalars(select(V2ExchangeOrder).where(V2ExchangeOrder.intent_id.in_(intent_ids))))
                if intent_ids
                else []
            )
            fills = (
                list(session.scalars(select(V2ExchangeFill).where(V2ExchangeFill.intent_id.in_(intent_ids))))
                if intent_ids
                else []
            )
            positions = (
                list(session.scalars(select(V2ManagedPosition).where(V2ManagedPosition.intent_id.in_(intent_ids))))
                if intent_ids
                else []
            )
            ...
            reconciliations = list(
                session.scalars(
                    select(V2ReconciliationSnapshot).where(
                        V2ReconciliationSnapshot.execution_mode == V2ExecutionMode.BINANCE_TESTNET.value
                    )
                )
            )
                ...
                context.update(
                    {
                        ...
                        "is_stop_order": bool(
                            protection and protection.stop_exchange_order_id == fill.exchange_order_id
                        ),
    ...
    try:
        with Session(engine) as session:
            ...
            for episode in episodes:
                ...
                bars = repository.list_ohlcv_bars(
                    symbol=episode.symbol, timeframe="1m", start_at=episode.entry_time, end_at=end_at
                )
                if bars:
...
def _unmatched_rows(
    ...
    local_by_trade: Mapping[tuple[str, str], Mapping[str, Any]],
    ...
    for symbol, trade_id in sorted(exchange_ids - local_ids):
        rows.append(
            {
                "record_type": "exchange_fill",
                "source": "Binance userTrades",
                "key": f"{symbol}/{trade_id}",
                "reason": "UNMANAGED_EXTERNAL_OR_LEGACY",
            }
        )
        ...
        rows.append(
            {
                ...
                "source": "local V2",
                "key": f"{symbol}/{trade_id}",
                "reason": "MISSING_FROM_EXCHANGE_USER_TRADES",
            }
        )
    local_order_ids = {
        str(row.get("exchange_order_id"))
        for row in local_facts
        if row.get("fact_type") == "order" and row.get("exchange_order_id")
    }
...
def audit(*, database_url: str, out_dir: Path, symbols: tuple[str, ...]) -> dict[str, Any]:
    ...
    exchange_trades = sorted(
        {(str(row.get("symbol")), str(row.get("id"))): row for row in exchange_trades}.values(),
        key=lambda row: (int(row.get("time", 0)), str(row.get("id", ""))),
    )
    first_trade_at = event_time(exchange_trades[0]) if exchange_trades else datetime.now(UTC)
    orders, order_fetch = _fetch_windowed(
        client, "fapiPrivateGetAllOrders", symbols, first_trade_at, include_symbol=True
    )
    income, income_fetch = _fetch_windowed(
        client, "fapiPrivateGetIncome", symbols, first_trade_at, include_symbol=False
    )
    income = [
        row for row in income if canonical_symbol(row.get("symbol")) in {canonical_symbol(symbol) for symbol in symbols}
    ]
    algo_orders, algo_fetch = _fetch_windowed(
        client, "fapiPrivateGetAllAlgoOrders", symbols, first_trade_at, include_symbol=True
    )
    ...
    episodes = build_trade_episodes(
        exchange_trades, funding=income, local_by_trade=local_by_trade, exit_reason_by_order=exit_reason_by_order
    )
        ...
        exchange_fill_rows.append(
            {
                ...
                "exchange_event_time": event_time(trade).isoformat(),
                "v2_matched": key in local_by_trade,
    ...
    required_fields_valid = all(
        all(
            key in trade and trade[key] not in (None, "")
            for key in ("id", "orderId", "qty", "price", "time", "commission")
        )
        for trade in exchange_trades
    )
    completeness = {
        ...
        "v2_local_fills": len(local_ids),
        "v2_matched_fills": len(local_ids & exchange_ids),
        "v2_match_rate": local_match_rate,
        "unmatched_records": len(unmatched),
        "mfe_mae_measured_episodes": mfe_coverage,
        ...
        "source_fetches": {
            "user_trades": trade_fetches,
            "orders": order_fetch,
            "income": income_fetch,
            "algo_orders": algo_fetch,
        },
        "local": local_metadata,
    }
    v2_episodes = [episode for episode in episodes if episode.local_context.get("strategy")]
    ...
    evaluation = {
        ...
        "best": {
            "symbol": _top_groups(v2_episodes, "symbol", reverse=True),
            "direction": _top_groups(v2_episodes, "direction", reverse=True),
            "strategy": _top_groups(v2_episodes, "strategy", reverse=True),
            "regime": _top_groups(v2_episodes, "regime", reverse=True),
            "score_bucket": _top_groups(v2_episodes, "score_bucket", reverse=True),
        },
        "worst": {
            "symbol": _top_groups(v2_episodes, "symbol", reverse=False),
            "direction": _top_groups(v2_episodes, "direction", reverse=False),
            "strategy": _top_groups(v2_episodes, "strategy", reverse=False),
            "regime": _top_groups(v2_episodes, "regime", reverse=False),
            "score_bucket": _top_groups(v2_episodes, "score_bucket", reverse=False),
        },
        "sl_tp": {
            "stop_hit": sum(episode.exit_reason == "STOP" for episode in v2_episodes),
            "tp_hit": sum(episode.exit_reason == "TAKE_PROFIT" for episode in v2_episodes),
            "mfe_measured": sum(episode.mfe_pct is not None for episode in v2_episodes),
            "mae_measured": sum(episode.mae_pct is not None for episode in v2_episodes),
        },
        "execution": {
            "reconciliation_defects": local_metadata["unhealthy_reconciliations"],
            "open_incidents": local_metadata["open_incidents"],
            "slippage": "NOT_MEASURED: no immutable submission reference price in all matched facts",
            "latency": "NOT_MEASURED: audit does not infer local wall-clock latency",
        },
    }
    (report_dir / "data_completeness.json").write_text(
        json.dumps(completeness, ensure_ascii=False, indent=2, default=_json_default) + "\n", encoding="utf-8"
    )
    (report_dir / "live_strategy_evaluation.json").write_text(
        json.dumps(evaluation, ensure_ascii=False, indent=2, default=_json_default) + "\n", encoding="utf-8"
    )
    (report_dir / "loss_attribution.json").write_text(
        json.dumps(loss_attribution, ensure_ascii=False, indent=2, default=_json_default) + "\n", encoding="utf-8"
    )
    markdown = "\n".join(
        [
            "# Live Actual Strategy Evaluation",
            "",
            f"- Data completeness: **{completeness['status']}**",
            f"- Scope: {', '.join(completeness['scope'])}",
            f"- Exchange trades: {completeness['exchange_trades']} (account total)",
            f"- Closed V2 trade episodes: {performance['trades']}",
            f"- V2 net PnL after commission and funding: {performance['net_pnl']} USDT",
            f"- V2 profit factor: {performance['profit_factor']}",
            f"- V2 expectancy: {performance['expectancy']} USDT",
            "",
            "## Loss Attribution",
            "",
            *[
                f"- {row['cause']}: {row['trades']} episode(s), {row['net_pnl']} USDT"
                for row in loss_attribution["top_loss_causes"]
            ],
            "",
...
def main() -> int:
    ...
    print(
        json.dumps(
            {
                "data_completeness": result["completeness"]["status"],
                "exchange_trades": result["completeness"]["exchange_trades"],
                "v2_match_rate": result["completeness"]["v2_match_rate"],
            },
            ensure_ascii=False,
        )
    )
