...
function AssetsTab({ navigate, strategies, drafts, ideas, materializeDraft, promoteIdea, createIdea, showIdeaForm, setShowIdeaForm, ideaForm, setIdeaForm, submitIdea }) {
  ...
  return <section className="assets-tab" role="tabpanel"><section className="form-row"><button type="button" onClick={() => setShowIdeaForm((current) => !current)}>{showIdeaForm ? "收起研究想法" : "登记研究想法"}</button></section>{showIdeaForm ? <section className="exchange-panel form-panel"><div className="panel-title"><h2>研究想法登记</h2></div><form className="form-grid" onSubmit={submitIdea}><input aria-label="想法标题" placeholder="想法标题" value={ideaForm.title} onChange={(event) => setIdeaForm((current) => ({ ...current, title: event.target.value }))} /><input aria-label="来源" placeholder="来源" value={ideaForm.source} onChange={(event) => setIdeaForm((current) => ({ ...current, source: event.target.value }))} /><input aria-label="交易标的" placeholder="交易标的" value={ideaForm.symbol} onChange={(event) => setIdeaForm((current) => ({ ...current, symbol: event.target.value }))} /><textarea aria-label="可验证核心假设" placeholder="可验证核心假设" value={ideaForm.hypothesis_summary} onChange={(event) => setIdeaForm((current) => ({ ...current, hypothesis_summary: event.target.value }))} /><textarea aria-label="研究依据" placeholder="研究依据（可选）" value={ideaForm.rationale} onChange={(event) => setIdeaForm((current) => ({ ...current, rationale: event.target.value }))} /><button type="submit" disabled={createIdea.isPending}>{createIdea.isPending ? "登记中" : "登记"}</button></form></section> : null}<section className="records-grid"><StrategyTable title="已物化策略" rows={strategyRows} columns={["strategy_id", "market", "timeframe", "risk_level", "paper_status"]} onRowClick={(row) => row.strategy_id && navigate(`/strategies/${row.strategy_id}`)} onLoading={strategies.isLoading} onError={strategies.isError ? strategies.error : null} /><StrategyTable title="规则草稿" rows={draftRows} columns={["draft_id", "source", "market", "timeframe", "draft_status"]} renderActions={(row) => <button type="button" onClick={() => materializeDraft.mutate(row.draft_id)} disabled={materializeDraft.isPending}>物化</button>} onLoading={drafts.isLoading} onError={drafts.isError ? drafts.error : null} /></section><section className="exchange-panel table-panel"><div className="panel-title"><h2>研究想法</h2><span>{ideaRows.length}</span></div><table><thead><tr><th>ID</th><th>来源</th><th>市场</th><th>状态</th><th>核心假设</th><th>操作</th></tr></thead><tbody>{ideaRows.length ? ideaRows.map((item) => <tr key={item.idea_id}><td>{item.idea_id}</td><td>{item.source}</td><td>{item.market}</td><td>{formatEnum(item.idea_status ?? item.intake_bucket, "-")}</td><td>{ideaSummary(item)}</td><td><button type="button" onClick={() => promoteIdea.mutate(item.idea_id)} disabled={promoteIdea.isPending}>晋升草稿</button></td></tr>) : ideas.isLoading ? <tr><td colSpan="6">正在加载…</td></tr> : ideas.isError ? <tr><td colSpan="6">加载失败：{ideas.error.message}</td></tr> : <tr><td colSpan="6">暂无策略想法。先登记一个可验证的研究想法。</td></tr>}</tbody></table></section></section>;
}

function StrategyTable({ title, rows, columns, onRowClick, renderActions, onLoading = false, onError = null }) { return <div className="exchange-panel table-panel"><div className="panel-title"><h2>{title}</h2><span>{rows.length}</span></div><table><thead><tr>{columns.map((column) => <th key={column}>{column}</th>)}{renderActions ? <th>操作</th> : null}</tr></thead><tbody>{onLoading ? <tr><td colSpan={columns.length + (renderActions ? 1 : 0)}>正在加载…</td></tr> : onError ? <tr><td colSpan={columns.length + (renderActions ? 1 : 0)}>加载失败：{onError.message ?? "服务暂不可用"}</td></tr> : rows.length ? rows.map((row, index) => <tr key={row.strategy_id ?? row.draft_id ?? index} className={onRowClick ? "clickable-row" : undefined} onClick={onRowClick ? () => onRowClick(row) : undefined}>{columns.map((column) => <td key={column}>{column === "risk_level" || column.endsWith("_status") ? formatEnum(row[column], "-") : String(row[column] ?? "-")}</td>)}{renderActions ? <td onClick={(event) => event.stopPropagation()}>{renderActions(row)}</td> : null}</tr>) : <tr><td colSpan={columns.length + (renderActions ? 1 : 0)}>暂无记录</td></tr>}</tbody></table></div>; }
